...
<div class="result">
    ...
    <div class="item">
        <div class="itemtop">
            <h4>
                <img alt="pdf" src="images/<%=fm.getTitle().split("\\.")[1]%>.png" class="doclogo">
                <%=fm.getTitle().split("\\.")[0]%>
            </h4>
            <h3>
                <a href="FileDownloadServlet?filename=<%=regexHtml.delHtmlTag(fm.getTitle())%>">点击下载</a>
            </h3>
        </div>
        <div class="itembuttom">
            <p><%=fm.getContent().length() > 210 ? fm.getContent().substring(0, 210) : fm.getContent()%>... </p>
        </div>
        <hr class="itemline">
    </div>
    <%
